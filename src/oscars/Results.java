...
public class Results {
    ...
    private final ImmutableMap<Column, ImmutableMap<String, String>> nomineeMap;

    private final Map<Column, ImmutableSet<String>> winners = Column.CATEGORIES.stream()
            .collect(Collectors.toMap(column -> column, column -> ImmutableSet.of()));
        ...
        try {
            ...
            awardsDOM.getChildren("category")
                    .forEach(categoryDOM -> winners.put(
                            Column.of(categoryDOM.getAttributeValue("name")),
                            categoryDOM.getChildren("nominee").stream()
    ...
    boolean prompt() throws IOException {
        ...
        for (int line = 0; line < Column.CATEGORIES.size() + ShowTimeType.values().length; line++)
            System.out.println(Font.menuNumber(line) + (line < Column.CATEGORIES.size()
                    ? toHeader(Column.CATEGORIES.get(line), false)
                    : toHeader(ShowTimeType.values()[line - Column.CATEGORIES.size()], false)));
    ...
    private String toHeader(Column inCategory, boolean inTitle) {
